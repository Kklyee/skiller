...
func Default() (Set, error) {
	...
	pins := filepath.Join(filepath.Dir(disabled), "pins.toml")
	if value, ok := os.LookupEnv(pinsFileEnv); ok && value != "" {
		pins = value
	}
	provenance := filepath.Join(filepath.Dir(disabled), "provenance.toml")
